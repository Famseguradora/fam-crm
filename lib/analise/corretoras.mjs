...
const DOMINIO_DA_CASA = /famseguradora/
const WEBMAIL = /^(gmail|hotmail|outlook|yahoo|live|icloud|uol|bol|terra|globo)\./

/** O nome inteiro colado, sem acento e sem espaço: "Abc Brasil Corretora" -> "abcbrasilcorretora". */
const colado = (s) => String(s ?? '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]/g, '')

/**
 * A CORRETORA DE UM E-MAIL, lida do e-mail inteiro  ·  10/09/2026
 *
 * Pedido dele: "ou o agente de triagem já identifica no corpo do e-mail, ou
 * então esse campo deve ser uma lista". Os dois: a lista é a tela, e isto é o
 * que chega preenchido nela.
 *
 * O REMETENTE NÃO SERVE: o pedido quase sempre chega ENCAMINHADO por alguém da
 * FAM (caso #18: "Comercial | FAM SEGURADORA"). Quem é a corretora está no meio
 * do corpo, na linha "De: Rosangela <rosangela.souza@abcbrasil.com.br>".
 *
 * Três degraus, e cada um só vale com UMA candidata (a mesma recusa do
 * `casarCorretora`: vínculo errado desce para o tomador e para a operação):
 *   1. o domínio de um endereço do e-mail é o domínio do e-mail CADASTRADO da
 *      corretora (97 das 99 têm e-mail no cadastro)
 *   2. o nome do domínio é o começo do nome da corretora ("genial.com.vc" ->
 *      "Genial Corretora de Seguros")
 *   3. o nome da corretora aparece escrito no texto (assinatura, "Corretora: X")
 *
 * @param {{ email_de?: string|null, corpo?: string|null, assunto?: string|null }} email
 * @param {(CorretoraCRM & { email?: string|null })[]} lista
 * @returns {Par & { dominio?: string }}
 */
export function acharCorretoraNoEmail(email, lista) {
  const texto = `${email?.email_de ?? ''}\n${email?.corpo ?? ''}`
  const vazio = { achou: false, nome_cru: '', nome: null, corretora_id: null, razao_social: null, cnpj: null, como: 'não achei a corretora no e-mail' }
  const cs = (lista ?? []).filter((c) => c?.id)

  const par = (c, como, dominio) => ({
    achou: true, nome_cru: dominio || '', nome: (c.nome_fantasia || '').trim() || c.razao_social,
    corretora_id: c.id, razao_social: c.razao_social, cnpj: c.cnpj ?? null, como, dominio,
  })
  const unica = (achadas) => {
    const ids = [...new Set(achadas.map((c) => c.id))]
    return ids.length === 1 ? achadas.find((c) => c.id === ids[0]) : null
  }

  const dominios = [...new Set((texto.match(/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}/gi) ?? [])
    .map((e) => e.split('@')[1].toLowerCase().replace(/[.>,;]+$/, '')))]
    .filter((d) => d && !DOMINIO_DA_CASA.test(d) && !WEBMAIL.test(d))

  // 1. o e-mail cadastrado da corretora
  for (const d of dominios) {
    const c = unica(cs.filter((x) => String(x.email ?? '').split('@')[1]?.toLowerCase().trim() === d))
    if (c) return par(c, `o e-mail ${d} é o domínio cadastrado da corretora`, d)
  }
  ...
  for (const d of dominios) {
    ...
    const c = unica(cs.filter((x) => [colado(x.razao_social), colado(x.nome_fantasia)].some((k) => k.length >= 4 && k.startsWith(base))))
    if (c) return par(c, `o domínio ${d} tem o nome da corretora`, d)
  }

  // 3. o nome escrito no texto
  const norm = ` ${String(email?.corpo ?? '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9]+/g, ' ')} `
  const escritas = cs.filter((x) => [x.nome_fantasia, x.razao_social].some((n) => {
    const ps = String(n ?? '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').replace(/[^a-z0-9\s]/g, ' ')
      .replace(RUIDO, ' ').split(/\s+/).filter(Boolean)
    return ps.length && ps.some((p) => p.length >= 4) && norm.includes(` ${ps.join(' ')} `)
  }))
  const c = unica(escritas)
  return c ? par(c, 'o nome da corretora está escrito no e-mail') : vazio
}
